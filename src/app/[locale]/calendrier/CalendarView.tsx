...
interface CalendarViewProps {
  initialYear: number
  initialMonth: number
  // The visible month's availability, computed on the server. The page used
  // to be a client component that rendered an empty grid and only started
  // fetching this after hydration — a guaranteed flash of "no slots" on every
  // visit, plus a Server Action round trip that the page render could have
  // covered for free.
  initialMonthSlots: Record<string, string[]>
}

export default function CalendarView({ initialYear, initialMonth, initialMonthSlots }: CalendarViewProps) {
  // The dictionary stays a hook here rather than a prop: `calendar` holds a
  // formatter function (timezoneNote) and functions can't cross the
  // server/client boundary.
  const locale = useLocale()
  const tc = useDictionary().calendar

  const [year, setYear] = useState(initialYear)
  const [month, setMonth] = useState(initialMonth)
  const [monthSlots, setMonthSlots] = useState<Record<string, string[]>>(initialMonthSlots)
  ...
  // The first month is already rendered from server data; only fetch once the
  // visitor actually navigates to a different one.
  const serverMonth = useRef(`${initialYear}-${initialMonth}`)
  useEffect(() => {
    if (serverMonth.current === `${year}-${month}`) return
    let cancelled = false
                ...
                <m.div key={selectedDay} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  {loadingDay ? (
                    ...
                    <div>
                      ...
                      {waitlistMessage ? (
                        ...
                        <form onSubmit={handleJoinWaitlist} className="space-y-2">
                          <input
                            required type="email" inputMode="email" autoComplete="email" value={waitlistEmail}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-3">
                      ...
                      <input required autoComplete="name" value={form.clientNom} onChange={(e) => setForm((p) => ({ ...p, clientNom: e.target.value }))} placeholder={tc.namePlaceholder} className="input text-sm" />
                      <input required type="email" inputMode="email" autoComplete="email" value={form.clientEmail} onChange={(e) => setForm((p) => ({ ...p, clientEmail: e.target.value }))} placeholder={tc.emailPlaceholder} className="input text-sm" />
                      <input type="tel" inputMode="tel" autoComplete="tel" value={form.clientTelephone} onChange={(e) => setForm((p) => ({ ...p, clientTelephone: e.target.value }))} maxLength={30} placeholder={tc.phonePlaceholder} className="input text-sm" />
