...
export default function ProjectsView({ projects, locale, t, statusLabel }: Props) {
  const categories = useMemo(
    () => [t.all, ...Array.from(new Set(projects.map((p) => p.category)))],
    [projects, t.all],
  )
  ...
  return (
    <div className="relative">
      ...
      {/* Filters */}
      <FadeIn>
        <div className="flex flex-wrap gap-2 mb-12" role="group" aria-label={t.filterAria}>
          {categories.map((cat) => (
            <button
              key={cat}
