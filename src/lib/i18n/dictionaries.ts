import type { Locale } from './locale'

const chatSuggestionsFr = [
  { label: 'Voir les projets', text: 'Quels sont tes projets les plus intéressants ?' },
  { label: 'Compétences & stack', text: 'Quelles sont tes compétences techniques ?' },
  { label: 'Combien coûte un site vitrine ?', text: 'Combien coûterait un site vitrine simple ?' },
  { label: 'Demander un devis', text: "J'aimerais obtenir un devis pour mon projet." },
]

const chatSuggestionsEn = [
  { label: 'View projects', text: 'What are your most interesting projects?' },
  { label: 'Skills & stack', text: 'What are your technical skills?' },
  { label: 'How much for a showcase website?', text: 'How much would a simple showcase website cost?' },
  { label: 'Request a quote', text: "I'd like to get a quote for my project." },
]

const fr = {
  error: {
    title: 'Une erreur est survenue',
    description: "Ce n'est pas grave — cliquez pour réessayer. Si le problème persiste, revenez à l'accueil.",
    retry: 'Réessayer',
    backHome: "Retour à l'accueil",
  },
  notFound: {
    label: 'Introuvable',
    title: 'Page non trouvée',
    description: "Ce contenu n'existe pas, ou n'est plus publié.",
    backHome: "Retour à l'accueil",
  },
  nav: {
    ariaMain: 'Navigation principale',
    home: 'Accueil',
    about: 'À propos',
    resume: 'Expérience',
    vision: 'Vision',
    projects: 'Projets',
    offers: 'Offres',
    calendar: 'Calendrier',
    blog: 'Blog',
    playground: 'Laboratoire',
    contact: 'Contact',
    openMenu: 'Ouvrir le menu',
    closeMenu: 'Fermer le menu',
    menuAria: 'Menu de navigation',
    switchTo: 'English',
    themeToggle: 'Basculer entre le mode sombre et le mode clair',
  },
  footer: {
    aria: 'Pied de page',
    navigation: 'Navigation',
    contactSocials: 'Contact & Réseaux',
    available: 'Disponible pour missions',
    rights: 'Tous droits réservés.',
    whatsappMessage:
      'Bonjour Nawaf 👋\n\nJe vous contacte via votre portfolio pour un projet que j’aimerais vous présenter. Je souhaiterais échanger avec vous afin de voir comment nous pourrions le concrétiser.\n\nMerci !',
    legalNotice: 'Mentions légales',
    privacyPolicy: 'Confidentialité',
  },
  status: {
    completed: 'Terminé',
    'in-progress': 'En cours',
    concept: 'Concept',
  } as Record<string, string>,
  home: {
    heroGreeting: 'Bonjour, je suis',
    heroRoleLabel: 'Développeur Fullstack & DevOps',
    heroAvailable: 'Disponible pour de nouvelles missions',
    nextSlotPrefix: 'Prochain créneau :',
    heroCtaProjects: 'Voir mes projets',
    heroCtaContact: 'Me contacter',
    heroScroll: 'SCROLL',
    heroScrollAria: 'Défiler vers la section À propos',
    aboutLabel: 'À propos',
    aboutTitleStart: 'Construire ce qui',
    aboutTitleHighlight: 'dure',
    aboutP1: "Avec plusieurs années d'expérience en développement web fullstack, j'ai acquis une expertise couvrant l'ensemble de la chaîne de valeur technique : de la conception d'interfaces jusqu'au déploiement automatisé sur des infrastructures cloud.",
    aboutP2: 'Ma singularité : je pense à la fois en développeur et en ingénieur infrastructure. Chaque ligne de code est pensée pour être déployée, monitorée et maintenue dans le temps.',
    aboutCta: 'Ma vision du métier',
    stats: [
      { value: '3+', label: "Ans d'expérience" },
      { value: '10+', label: 'Projets livrés' },
      { value: '15+', label: 'Technologies' },
      { value: '100%', label: 'Implication' },
    ],
    stackLabel: 'Stack technique',
    stackTitle: 'Technologies maîtrisées',
    testimonialsLabel: "Ce qu'ils disent",
    testimonialsTitle: 'Témoignages',
    testimonialsCta: 'Partager mon expérience',
    testimonialsVerified: 'Vérifié — voir le projet',
    testimonialsEmpty: 'Soyez le premier à partager votre expérience.',
    ctaLabel: 'Disponible',
    ctaTitle: 'Envie de collaborer ?',
    ctaText: "Je suis disponible pour des missions freelance, des collaborations ou simplement discuter d'un projet ambitieux.",
    ctaButton1: 'Me contacter',
    ctaButton2: 'Envoyer un message',
  },
  resume: {
    label: 'Parcours',
    title: 'Mon Expérience',
    subtitle: 'Parcours professionnel, compétences techniques et formations.',
    experiencesLabel: 'Expériences professionnelles',
    skillsLabel: 'Compétences techniques',
    educationLabel: 'Formations',
    downloadCv: 'Télécharger mon CV',
  },
  vision: {
    label: 'Philosophie',
    title: 'Ma Vision',
    philosophyLabel: 'Philosophie',
    approachLabel: 'Approche',
    valuesLabel: 'Valeurs fondamentales',
  },
  projects: {
    label: 'Portfolio',
    title: 'Mes Projets',
    subtitle: 'Réalisations web, DevOps et interfaces — du prototype à la production.',
    filterAria: 'Filtrer par catégorie',
    all: 'Tous',
    searchPlaceholder: 'Rechercher un projet…',
    searchAria: 'Rechercher un projet',
    viewProject: 'Voir le projet',
    empty: 'Aucun projet dans cette catégorie.',
    noResults: 'Aucun résultat pour cette recherche.',
    allProjectsLink: 'Tous les projets',
    technologiesUsed: 'Technologies utilisées',
    previewUnavailable: 'Aperçu non disponible',
    links: 'Liens',
    viewLive: 'Voir en ligne',
    viewSource: 'Code source',
    infoLabel: 'Informations',
    category: 'Catégorie',
    year: 'Année',
    statusLabel: 'Statut',
    viewAllProjects: 'Voir tous les projets',
    caseStudyLabel: 'Étude de cas',
    caseStudyContext: 'Contexte',
    caseStudySolution: 'Solution',
    caseStudyResults: 'Résultats',
  },
  blog: {
    label: 'Articles',
    title: 'Blog',
    subtitle: "Retours d'expérience, réflexions techniques et pensées sur le développement moderne.",
    all: 'Tous',
    filterAria: 'Filtrer par catégorie',
    searchPlaceholder: 'Rechercher un article…',
    searchAria: 'Rechercher un article',
    noResults: 'Aucun résultat pour cette recherche.',
    emptyTitle: 'Premiers articles en cours de rédaction',
    emptyText: 'Revenez bientôt pour des articles sur le développement fullstack et DevOps.',
    readTime: 'de lecture',
    backToBlog: 'Blog',
    allArticles: '← Tous les articles',
    contact: 'Me contacter',
    resourceMentioned: 'Ressource mentionnée',
    discover: 'Découvrir',
  },
  devis: {
    label: 'Devis en ligne',
    title: 'Obtenir un devis',
    subtitle: "Sélectionnez les prestations qui vous intéressent, décrivez votre projet, recevez votre devis instantanément — sans passer par le chat.",
    formTitle: 'Nouveau devis',
    nameLabel: 'Nom complet',
    namePlaceholder: 'Votre nom et prénom',
    companyLabel: 'Société',
    companyPlaceholder: 'Nom de votre société',
    addressLabel: 'Adresse',
    addressPlaceholder: 'Votre adresse',
    emailLabel: 'Email',
    emailPlaceholder: 'votre@email.com',
    phoneLabel: 'Téléphone',
    phonePlaceholder: '+241 XX XX XX XX',
    descriptionLabel: 'Décrivez votre projet',
    descriptionPlaceholder: 'Un site vitrine pour mon activité de…',
    offersLabel: 'Prestations souhaitées',
    noOffers: "Aucune prestation n'est disponible pour un devis instantané pour le moment — contactez-moi directement.",
    quantityLabel: 'Qté',
    complexityLabel: 'Complexité',
    tierSimple: 'Simple',
    tierStandard: 'Standard',
    tierComplexe: 'Complexe',
    rangeNote: 'Le prix dépend de la complexité choisie ci-dessous pour chaque prestation — le montant final peut être ajusté après échange.',
    totalPreviewLabel: 'Total estimé HT',
    submit: 'Générer mon devis',
    submitting: 'Génération…',
    errorRequired: 'Merci de renseigner tous les champs et de choisir au moins une prestation.',
    errorGeneric: 'Une erreur est survenue. Merci de réessayer.',
    lookupTitle: 'Retrouver un devis existant',
    lookupPlaceholder: 'Numéro ou code de suivi',
    lookupButton: 'Rechercher',
    lookupSearching: 'Recherche…',
    lookupNotFound: 'Aucun devis trouvé pour cette référence.',
    newQuoteButton: 'Faire un nouveau devis',
  },
  playground: {
    label: 'Bac à sable',
    title: 'Laboratoire',
    subtitle: "Un éditeur JavaScript qui tourne entièrement dans votre navigateur — modifiez le code, cliquez sur Exécuter, rien n'est envoyé à un serveur.",
    filename: 'playground.js',
    run: 'Exécuter',
    running: 'Exécution…',
    reset: 'Réinitialiser',
    outputLabel: 'Console',
    outputEmpty: 'Le résultat de votre code s’affichera ici.',
    timeoutMessage: 'Exécution interrompue : trop de temps écoulé (boucle infinie ?).',
    statusDone: 'Terminé',
    statusError: 'Erreur',
    shortcutHint: 'Astuce : Ctrl+Entrée pour exécuter',
    starterCode: `const prices = [15000, 25000, 10000, 30000];

function calculateTotal(prices) {
  const total = prices.reduce((sum, price) => sum + price, 0);
  return total;
}

const total = calculateTotal(prices);
const discount = total * 0.1;
const finalPrice = total - discount;

console.log("Prix :", prices);
console.log("Total :", total + " FCFA");
console.log("Réduction :", discount + " FCFA");
console.log("Prix final :", finalPrice + " FCFA");
`,
  },
  contact: {
    label: 'Discutons',
    title: 'Contact',
    subtitle: 'Une idée, un projet, une question ? Je réponds à tous les messages.',
    email: 'Email',
    location: 'Localisation',
    locationValue: 'Libreville, Gabon — Disponible en remote',
    availability: 'Disponibilité',
    availableMissions: 'Disponible pour de nouvelles missions',
    responseTime: 'Réponse sous 48h ouvrables.',
    form: {
      nameLabel: 'Nom complet *',
      namePlaceholder: 'Prénom Nom',
      emailLabel: 'Email *',
      emailPlaceholder: 'adresse@email.com',
      emailInvalid: 'Format invalide — ex: prenom@domaine.com',
      phoneLabel: 'Téléphone',
      phoneOptional: '(optionnel)',
      phonePlaceholder: '+241 ...',
      subjectLabel: 'Sujet *',
      subjectPlaceholder: 'Sélectionner un sujet',
      subjectMission: 'Mission / Projet freelance',
      subjectCollaboration: 'Collaboration',
      subjectConseil: 'Conseil technique',
      subjectAutre: 'Autre',
      customSubjectLabel: "Précisez l'objet *",
      customSubjectPlaceholder: "Indiquez l'objet de votre message",
      messageLabel: 'Message *',
      messagePlaceholder: 'Décrivez votre besoin, vos contraintes, vos objectifs…',
      sending: 'Envoi en cours…',
      send: 'Envoyer le message',
      successTitle: 'Message envoyé !',
      successText: 'Merci — je vous répondrai sous 48 heures ouvrables.',
    },
  },
  offers: {
    label: 'Tarifs',
    title: 'Nos Offres',
    subtitle: "Des solutions conçues pour répondre à vos besoins, avec des tarifs indicatifs adaptés à chaque type de projet. Chaque prestation est entièrement personnalisable et fait l'objet d'un devis sur mesure. Les prix affichés sont des fourchettes estimatives et peuvent être ajustés en fonction de vos exigences, de votre budget et de la complexité du projet.",
    specialTitle: 'Offres Spéciales',
    cta: 'Demander un devis',
    ctaMessage: (title: string) => `J'aimerais un devis pour : ${title}`,
    onlineQuoteCta: 'Obtenir un devis instantané en ligne',
    empty: 'Aucune offre pour le moment.',
    heroGoTo: (title: string) => `Aller à l'offre : ${title}`,
    heroPause: 'Mettre en pause le défilement',
    heroPlay: 'Reprendre le défilement',
  },
  calendar: {
    label: 'Rendez-vous',
    title: 'Prendre rendez-vous',
    subtitle: 'Réservez un créneau pour discuter de votre projet — je vous recontacte par email pour confirmer les détails.',
    pickDay: 'Sélectionnez un jour disponible sur le calendrier.',
    timezoneNote: (tz: string) => `Heures affichées dans votre fuseau horaire (${tz}).`,
    noSlots: 'Aucun créneau disponible ce jour-là.',
    fullyBooked: 'Complet pour ce jour — vous pouvez être averti si une place se libère.',
    joinWaitlist: 'Être averti si une place se libère',
    slotTaken: 'Déjà réservé',
    namePlaceholder: 'Votre nom',
    emailPlaceholder: 'Votre email',
    phonePlaceholder: 'Votre téléphone (optionnel)',
    messagePlaceholder: 'Un mot sur votre projet (optionnel)',
    confirm: 'Confirmer le rendez-vous',
    booking: 'Réservation…',
    confirmedTitle: 'Rendez-vous confirmé !',
    confirmedBody: 'Un email de confirmation avec un fichier calendrier vous a été envoyé.',
    accessCodeLabel: 'Code de suivi',
    meetingLinkLabel: 'Visioconférence',
    joinMeeting: 'Rejoindre la visio',
    genericError: 'Une erreur est survenue. Merci de réessayer.',
  },
  testimonialPage: {
    label: 'Votre avis compte',
    title: 'Partagez votre expérience',
    subtitle: "Vous avez travaillé avec moi ? Votre témoignage aide d'autres clients à me faire confiance. Il sera publié sur le site après validation.",
    form: {
      nameLabel: 'Nom complet *',
      namePlaceholder: 'Prénom Nom',
      roleLabel: 'Poste',
      rolePlaceholder: 'Directeur Technique',
      companyLabel: 'Entreprise',
      companyPlaceholder: "Nom de l'entreprise",
      ratingLabel: 'Votre note *',
      textLabel: 'Votre témoignage *',
      textPlaceholder: 'Décrivez votre expérience de collaboration…',
      sending: 'Envoi en cours…',
      send: 'Envoyer mon témoignage',
      successTitle: 'Merci pour votre témoignage !',
      successText: 'Il sera publié sur le site après validation par Nawaf.',
    },
  },
  chat: {
    toggleOpen: 'Ouvrir le chat',
    toggleClose: 'Fermer le chat',
    panelAria: 'Assistant du portfolio',
    headerTitle: 'Nem CopilotIA',
    headerStatus: 'En ligne',
    clearChat: 'Effacer la conversation',
    greeting: 'Bonjour ! Posez-moi une question sur le parcours, les projets ou les compétences de Nawaf. Je peux aussi',
    greetingBold: 'générer un devis personnalisé',
    greetingEnd: 'pour votre projet, directement dans la conversation !',
    suggestions: chatSuggestionsFr,
    generatingQuote: 'Génération du devis…',
    searchingQuote: 'Recherche du devis…',
    sendingEmail: "Envoi de l'email…",
    checkingAvailability: 'Vérification des disponibilités…',
    bookingMeeting: 'Réservation du rendez-vous…',
    searchingBooking: 'Recherche du rendez-vous…',
    cancelingBooking: 'Annulation du rendez-vous…',
    joiningWaitlist: "Inscription sur la liste d'attente…",
    requestingHuman: 'Envoi de la demande…',
    humanHelpTitle: 'Laissez vos coordonnées pour que Nawaf puisse vous recontacter :',
    humanHelpNamePlaceholder: 'Votre nom',
    humanHelpEmailPlaceholder: 'Votre email',
    humanHelpPhonePlaceholder: 'Votre téléphone (optionnel)',
    humanHelpSubmit: 'Envoyer',
    noAvailability: 'Aucun créneau disponible pour le moment.',
    bookingPrefix: 'Rendez-vous',
    bookingCancelled: 'Annulé',
    thinkingAria: "L'assistant réfléchit…",
    thinking: 'réfléchit',
    placeholder: 'Écrivez votre question…',
    inputAria: 'Votre message',
    sendAria: 'Envoyer',
    quotePrefix: 'Devis',
    totalTTC: 'Total TTC',
    trackingCode: 'Code de suivi',
    joinMeeting: 'Rejoindre la visio',
    viewPrintQuote: 'Voir et imprimer le devis',
    signOnline: 'Signer en ligne',
    requestCall: 'Demander un appel',
    callSubject: 'Rendez-vous — devis',
    callBodyIntro: "Bonjour,\n\nJe souhaite convenir d'un appel au sujet du devis",
    callBodyOutro: 'Mes disponibilités :\n\nMerci !',
    cvSuggestion: { label: 'Télécharger le CV', text: 'Peux-tu me faire parvenir ton CV ?' },
  },
}

const en: typeof fr = {
  error: {
    title: 'Something went wrong',
    description: "It's not serious — click to try again. If the problem persists, head back home.",
    retry: 'Try again',
    backHome: 'Back to home',
  },
  notFound: {
    label: 'Not found',
    title: 'Page not found',
    description: "This content doesn't exist, or isn't published anymore.",
    backHome: 'Back to home',
  },
  nav: {
    ariaMain: 'Main navigation',
    home: 'Home',
    about: 'About',
    resume: 'Experience',
    vision: 'Vision',
    projects: 'Projects',
    offers: 'Offers',
    calendar: 'Calendar',
    blog: 'Blog',
    playground: 'Playground',
    contact: 'Contact',
    openMenu: 'Open menu',
    closeMenu: 'Close menu',
    menuAria: 'Navigation menu',
    switchTo: 'Français',
    themeToggle: 'Toggle dark and light mode',
  },
  footer: {
    aria: 'Footer',
    navigation: 'Navigation',
    contactSocials: 'Contact & Socials',
    available: 'Available for work',
    rights: 'All rights reserved.',
    whatsappMessage:
      "Hi Nawaf 👋\n\nI'm reaching out via your portfolio about a project I'd like to present to you. I'd love to chat and see how we could bring it to life.\n\nThanks!",
    legalNotice: 'Legal notice',
    privacyPolicy: 'Privacy',
  },
  status: {
    completed: 'Completed',
    'in-progress': 'In progress',
    concept: 'Concept',
  },
  home: {
    heroGreeting: "Hi, I'm",
    heroRoleLabel: 'Fullstack & DevOps Developer',
    heroAvailable: 'Available for new opportunities',
    nextSlotPrefix: 'Next slot:',
    heroCtaProjects: 'View my projects',
    heroCtaContact: 'Contact me',
    heroScroll: 'SCROLL',
    heroScrollAria: 'Scroll to the About section',
    aboutLabel: 'About',
    aboutTitleStart: 'Building things that',
    aboutTitleHighlight: 'last',
    aboutP1: "With several years of experience in fullstack web development, I've built expertise spanning the entire technical value chain: from interface design to automated deployment on cloud infrastructure.",
    aboutP2: 'My edge: I think like both a developer and an infrastructure engineer. Every line of code is written to be deployed, monitored and maintained over time.',
    aboutCta: 'My take on the craft',
    stats: [
      { value: '3+', label: 'Years of experience' },
      { value: '10+', label: 'Projects shipped' },
      { value: '15+', label: 'Technologies' },
      { value: '100%', label: 'Commitment' },
    ],
    stackLabel: 'Tech stack',
    stackTitle: 'Technologies I use',
    testimonialsLabel: 'What people say',
    testimonialsTitle: 'Testimonials',
    testimonialsCta: 'Share your experience',
    testimonialsVerified: 'Verified — see the project',
    testimonialsEmpty: 'Be the first to share your experience.',
    ctaLabel: 'Available',
    ctaTitle: 'Want to work together?',
    ctaText: "I'm available for freelance work, collaborations, or just to talk about an ambitious project.",
    ctaButton1: 'Contact me',
    ctaButton2: 'Send a message',
  },
  resume: {
    label: 'Background',
    title: 'My Experience',
    subtitle: 'Professional background, technical skills and education.',
    experiencesLabel: 'Work experience',
    skillsLabel: 'Technical skills',
    educationLabel: 'Education',
    downloadCv: 'Download my CV',
  },
  vision: {
    label: 'Philosophy',
    title: 'My Vision',
    philosophyLabel: 'Philosophy',
    approachLabel: 'Approach',
    valuesLabel: 'Core values',
  },
  projects: {
    label: 'Portfolio',
    title: 'My Projects',
    subtitle: 'Web, DevOps and interface work — from prototype to production.',
    filterAria: 'Filter by category',
    all: 'All',
    searchPlaceholder: 'Search projects…',
    searchAria: 'Search projects',
    viewProject: 'View project',
    empty: 'No projects in this category.',
    noResults: 'No results for this search.',
    allProjectsLink: 'All projects',
    technologiesUsed: 'Technologies used',
    previewUnavailable: 'Preview unavailable',
    links: 'Links',
    viewLive: 'View live',
    viewSource: 'Source code',
    infoLabel: 'Information',
    category: 'Category',
    year: 'Year',
    statusLabel: 'Status',
    viewAllProjects: 'View all projects',
    caseStudyLabel: 'Case study',
    caseStudyContext: 'Context',
    caseStudySolution: 'Solution',
    caseStudyResults: 'Results',
  },
  blog: {
    label: 'Articles',
    title: 'Blog',
    subtitle: 'Lessons learned, technical thoughts and reflections on modern development.',
    all: 'All',
    filterAria: 'Filter by category',
    searchPlaceholder: 'Search articles…',
    searchAria: 'Search articles',
    noResults: 'No results for this search.',
    emptyTitle: 'First articles coming soon',
    emptyText: 'Check back soon for articles on fullstack development and DevOps.',
    readTime: 'read',
    backToBlog: 'Blog',
    allArticles: '← All articles',
    contact: 'Contact me',
    resourceMentioned: 'Resource mentioned',
    discover: 'Discover',
  },
  devis: {
    label: 'Online quote',
    title: 'Get a quote',
    subtitle: "Pick the services you're interested in, describe your project, get your quote instantly — no chat required.",
    formTitle: 'New quote',
    nameLabel: 'Full name',
    namePlaceholder: 'Your full name',
    companyLabel: 'Company',
    companyPlaceholder: 'Your company name',
    addressLabel: 'Address',
    addressPlaceholder: 'Your address',
    emailLabel: 'Email',
    emailPlaceholder: 'your@email.com',
    phoneLabel: 'Phone',
    phonePlaceholder: '+241 XX XX XX XX',
    descriptionLabel: 'Describe your project',
    descriptionPlaceholder: 'A showcase website for my…',
    offersLabel: 'Services wanted',
    noOffers: 'No service is available for an instant quote right now — contact me directly.',
    quantityLabel: 'Qty',
    complexityLabel: 'Complexity',
    tierSimple: 'Simple',
    tierStandard: 'Standard',
    tierComplexe: 'Complex',
    rangeNote: "The price depends on the complexity picked below for each service — the final amount may be adjusted after we talk.",
    totalPreviewLabel: 'Estimated total (excl. tax)',
    submit: 'Generate my quote',
    submitting: 'Generating…',
    errorRequired: 'Please fill in every field and pick at least one service.',
    errorGeneric: 'Something went wrong. Please try again.',
    lookupTitle: 'Find an existing quote',
    lookupPlaceholder: 'Number or tracking code',
    lookupButton: 'Search',
    lookupSearching: 'Searching…',
    lookupNotFound: 'No quote found for this reference.',
    newQuoteButton: 'Start a new quote',
  },
  playground: {
    label: 'Sandbox',
    title: 'Playground',
    subtitle: 'A JavaScript editor that runs entirely in your browser — edit the code, hit Run, nothing is ever sent to a server.',
    filename: 'playground.js',
    run: 'Run',
    running: 'Running…',
    reset: 'Reset',
    outputLabel: 'Console',
    outputEmpty: 'Your code’s output will show up here.',
    timeoutMessage: 'Execution stopped: took too long (infinite loop?).',
    statusDone: 'Done',
    statusError: 'Error',
    shortcutHint: 'Tip: Ctrl+Enter to run',
    starterCode: `const prices = [15000, 25000, 10000, 30000];

function calculateTotal(prices) {
  const total = prices.reduce((sum, price) => sum + price, 0);
  return total;
}

const total = calculateTotal(prices);
const discount = total * 0.1;
const finalPrice = total - discount;

console.log("Prices:", prices);
console.log("Total:", total + " FCFA");
console.log("Discount:", discount + " FCFA");
console.log("Final price:", finalPrice + " FCFA");
`,
  },
  contact: {
    label: "Let's talk",
    title: 'Contact',
    subtitle: 'An idea, a project, a question? I reply to every message.',
    email: 'Email',
    location: 'Location',
    locationValue: 'Libreville, Gabon — Available remotely',
    availability: 'Availability',
    availableMissions: 'Available for new opportunities',
    responseTime: 'Response within 48 business hours.',
    form: {
      nameLabel: 'Full name *',
      namePlaceholder: 'First Last',
      emailLabel: 'Email *',
      emailPlaceholder: 'you@email.com',
      emailInvalid: 'Invalid format — e.g. name@domain.com',
      phoneLabel: 'Phone',
      phoneOptional: '(optional)',
      phonePlaceholder: '+1 ...',
      subjectLabel: 'Subject *',
      subjectPlaceholder: 'Select a subject',
      subjectMission: 'Freelance mission / Project',
      subjectCollaboration: 'Collaboration',
      subjectConseil: 'Technical advice',
      subjectAutre: 'Other',
      customSubjectLabel: 'Specify the subject *',
      customSubjectPlaceholder: 'Enter the subject of your message',
      messageLabel: 'Message *',
      messagePlaceholder: 'Describe your needs, constraints, goals…',
      sending: 'Sending…',
      send: 'Send message',
      successTitle: 'Message sent!',
      successText: "Thanks — I'll get back to you within 48 business hours.",
    },
  },
  offers: {
    label: 'Pricing',
    title: 'Our Offers',
    subtitle: 'Solutions designed around your needs, with indicative rates tailored to each type of project. Every service is fully customizable and quoted individually. The prices shown are estimated ranges and can be adjusted based on your requirements, budget, and project complexity.',
    specialTitle: 'Special Offers',
    cta: 'Request a quote',
    ctaMessage: (title: string) => `I'd like a quote for: ${title}`,
    onlineQuoteCta: 'Get an instant online quote',
    empty: 'No offers yet.',
    heroGoTo: (title: string) => `Go to offer: ${title}`,
    heroPause: 'Pause the slideshow',
    heroPlay: 'Resume the slideshow',
  },
  calendar: {
    label: 'Appointments',
    title: 'Book a call',
    subtitle: "Reserve a slot to discuss your project — I'll follow up by email to confirm the details.",
    pickDay: 'Select an available day on the calendar.',
    timezoneNote: (tz: string) => `Times shown in your timezone (${tz}).`,
    noSlots: 'No slots available that day.',
    fullyBooked: 'Fully booked that day — you can be notified if a slot opens up.',
    joinWaitlist: 'Notify me if a slot opens up',
    slotTaken: 'Already booked',
    namePlaceholder: 'Your name',
    emailPlaceholder: 'Your email',
    phonePlaceholder: 'Your phone (optional)',
    messagePlaceholder: 'A note about your project (optional)',
    confirm: 'Confirm the booking',
    booking: 'Booking…',
    confirmedTitle: 'Booking confirmed!',
    confirmedBody: "A confirmation email with a calendar file has been sent to you.",
    accessCodeLabel: 'Tracking code',
    meetingLinkLabel: 'Video call',
    joinMeeting: 'Join the video call',
    genericError: 'Something went wrong. Please try again.',
  },
  testimonialPage: {
    label: 'Your feedback matters',
    title: 'Share your experience',
    subtitle: "Worked with me? Your testimonial helps other clients trust me. It will be published on the site after review.",
    form: {
      nameLabel: 'Full name *',
      namePlaceholder: 'First Last',
      roleLabel: 'Role',
      rolePlaceholder: 'CTO',
      companyLabel: 'Company',
      companyPlaceholder: 'Company name',
      ratingLabel: 'Your rating *',
      textLabel: 'Your testimonial *',
      textPlaceholder: 'Describe your experience working together…',
      sending: 'Sending…',
      send: 'Send my testimonial',
      successTitle: 'Thanks for your testimonial!',
      successText: "It will be published on the site once Nawaf reviews it.",
    },
  },
  chat: {
    toggleOpen: 'Open chat',
    toggleClose: 'Close chat',
    panelAria: 'Portfolio assistant',
    headerTitle: 'Nem CopilotIA',
    headerStatus: 'Online',
    clearChat: 'Clear conversation',
    greeting: 'Hi! Ask me anything about Nawaf’s background, projects or skills. I can also',
    greetingBold: 'generate a custom quote',
    greetingEnd: 'for your project, right here in the chat!',
    suggestions: chatSuggestionsEn,
    generatingQuote: 'Generating the quote…',
    searchingQuote: 'Looking up the quote…',
    sendingEmail: 'Sending the email…',
    checkingAvailability: 'Checking availability…',
    bookingMeeting: 'Booking the meeting…',
    searchingBooking: 'Looking up the booking…',
    cancelingBooking: 'Cancelling the booking…',
    joiningWaitlist: 'Joining the waitlist…',
    requestingHuman: 'Sending the request…',
    humanHelpTitle: 'Leave your details so Nawaf can get back to you:',
    humanHelpNamePlaceholder: 'Your name',
    humanHelpEmailPlaceholder: 'Your email',
    humanHelpPhonePlaceholder: 'Your phone (optional)',
    humanHelpSubmit: 'Send',
    noAvailability: 'No slots available right now.',
    bookingPrefix: 'Booking',
    bookingCancelled: 'Cancelled',
    thinkingAria: 'The assistant is thinking…',
    thinking: 'thinking',
    placeholder: 'Type your question…',
    inputAria: 'Your message',
    sendAria: 'Send',
    quotePrefix: 'Quote',
    totalTTC: 'Total (incl. tax)',
    trackingCode: 'Tracking code',
    joinMeeting: 'Join the video call',
    viewPrintQuote: 'View and print the quote',
    signOnline: 'Sign online',
    requestCall: 'Request a call',
    callSubject: 'Meeting — quote',
    callBodyIntro: "Hello,\n\nI'd like to schedule a call about quote",
    callBodyOutro: 'My availability:\n\nThanks!',
    cvSuggestion: { label: 'Download CV', text: 'Could you send me your CV?' },
  },
}

export const dictionaries = { fr, en } satisfies Record<Locale, typeof fr>

export type Dictionary = typeof fr

export function getDictionary(locale: Locale): Dictionary {
  return dictionaries[locale]
}
